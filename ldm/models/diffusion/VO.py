...
import math
from ldm.models.diffusion.loss import  caculate_loss_att_fixed_cnt
class VOSampler(object):
    ...
    def sample(self, S, shape, input, uc=None, guidance_scale=1, mask=None, x0=None, loss_type=None):
        ...
        return self.VO_sampling(shape, input, uc, guidance_scale, mask=mask, x0=x0, loss_type=loss_type)


    # @torch.no_grad()
    def VO_sampling(self, shape, input, uc=None, guidance_scale=1, mask=None, x0=None, loss_type=None):
        ...
        for i, step in enumerate(time_range):
            ...
            if input['object_position'] != []:
                x = self.update_loss( input,i, index, ts )
                input["x"] = x
            
            img, pred_x0, e_t = self.p_sample_VO(input, ts, index=index, uc=uc, guidance_scale=guidance_scale, old_eps=old_eps, t_next=ts_next)
            input["x"] = img
            old_eps.append(e_t)
            if len(old_eps) >= 4:
                old_eps.pop(0)
        ...
        return img,img_list,x0_list

    def update_loss(self, input,index1, index, ts,type_loss=None):
        ...
        if (index1 < max_index):
            while (iteration < max_iter and (index1 < max_index)) : #or max_outside>0.15
                ...
                x = x.detach()
                iteration += 1
                del loss2, att_first, att_second, att_third, self_first, self_second, self_third, ground1, ground2, ground3
    ...
    @torch.no_grad()
    def p_sample_VO(self, input, t, index, guidance_scale=1., uc=None, old_eps=None, t_next=None):
        x = deepcopy(input["x"]) 
        b = x.shape[0]
